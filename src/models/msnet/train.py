...
from src.models.msnet.util.MSNet import MSNet
...
def train(config_file):
    ...
    x = tf.compat.v1.placeholder(tf.float32, shape=full_data_shape)
    w = tf.compat.v1.placeholder(tf.float32, shape=full_label_shape)
    y = tf.compat.v1.placeholder(tf.int64, shape=full_label_shape)
    ...
    for n in range(start_it, config_train["maximal_iteration"]):
        ...
        if (n + 1) % config_train["snapshot_iteration"] == 0:
            saver.save(
                sess, config_train["model_save_prefix"] + "_{0:}.ckpt".format(n + 1)
            )
    sess.close()
