...
def load_3d_volume_as_array(filename):
    if NiftiExtensions.NIFTI.value in filename:
...
def crop_ND_volume_with_bounding_box(volume, min_idx, max_idx):
    ...
    if dim == 2:
        output = volume[
            np.ix_(range(min_idx[0], max_idx[0] + 1), range(min_idx[1], max_idx[1] + 1))
        ]
    elif dim == 3:
        output = volume[
            np.ix_(
                range(min_idx[0], max_idx[0] + 1),
...
def set_ND_volume_roi_with_bounding_box_range(volume, bb_min, bb_max, sub_volume):
    ...
    if dim == 2:
        out[
            np.ix_(range(bb_min[0], bb_max[0] + 1), range(bb_min[1], bb_max[1] + 1))
        ] = sub_volume
    elif dim == 3:
        out[
            np.ix_(
                range(bb_min[0], bb_max[0] + 1),
...
def get_random_roi_sampling_center(
    input_shape, output_shape, sample_mode, bounding_box=None
):
    """
...
def set_roi_to_volume(volume, center, sub_volume):
    ...
    if len(center) == 3:
        output_volume[
            np.ix_(
                range(center[0] - r0[0], center[0] + r1[0]),
                range(center[1] - r0[1], center[1] + r1[1]),
                range(center[2] - r0[2], center[2] + r1[2]),
            )
        ] = sub_volume[
            np.ix_(
                range(patch_center[0] - r0[0], patch_center[0] + r1[0]),
