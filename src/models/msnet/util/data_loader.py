...
from src.common.enums import NiftiExtensions
...
class DataLoader:
    def __init__(self, config):
        ...
        self.data_root = (
            config["data_root"]
            if type(config["data_root"]) is list
            else [config["data_root"]]
        )
        self.modality_postfix = config.get(
            "modality_postfix", ["flair", "t1", "t1ce", "t2"]
        )
        self.intensity_normalize = config.get(
        ...
        self.with_flip = config.get("with_flip", True)
    ...
    def __load_one_volume(self, patient_name, mod):
        ...
        if "nii" in self.file_postfix:
            ...
            for image_name in image_names:
                if (mod in image_name) and (NiftiExtensions.NIFTI.value in image_name):
        ...
        # for brats15
            ...
            for img_file_dir in img_file_dirs:
                if mod + "." in img_file_dir:
                    volume_name = (
                        img_file_dir + "/" + img_file_dir + "." + self.file_postfix
                    )
                    break
    ...
    def load_data(self):
        ...
        in_size = []
        data_num = (
            self.data_num if (self.data_num is not None) else len(self.patient_names)
        )
        for i in range(data_num):
            ...
            for mod_idx in range(len(self.modality_postfix)):
                print(
                    "Loading patient {} and modality {}".format(
                        self.patient_names[i], self.modality_postfix[mod_idx]
                    )
                )
                volume, volume_name = self.__load_one_volume(
                    self.patient_names[i], self.modality_postfix[mod_idx]
                )
                if mod_idx == 0:
                    margin = 5
                ...
                if self.data_resize:
                    volume = resize_3D_volume_to_given_shape(
                        volume, self.data_resize, 1
                    )
                if mod_idx == 0:
            ...
            if self.with_ground_truth:
                label, _ = self.__load_one_volume(
                    self.patient_names[i], self.label_postfix
                )
    ...
    def __get_one_batch(self):
        ...
        batch_sample_model = self.config.get(
            "batch_sample_model", ("full", "valid", "valid")
        )
        batch_slice_direction = self.config.get(
            "batch_slice_direction", "axial"
        )  # axial, sagittal, coronal or random
        ...
        for i in range(batch_size):
            ...
            if self.with_ground_truth:
                ...
                if train_with_roi_patch:
                    ...
                    if keep_roi_outside:
                        ...
                        for idx in range(len(data_volumes)):
                            data_volumes[idx] = data_volumes[idx][
                                np.ix_(
                                    range(mind, maxd),
                                    range(minh, maxh),
                                    range(minw, maxw),
                        ...
                        for idx in range(len(weight_volumes)):
                            weight_volumes[idx] = weight_volumes[idx][
                                np.ix_(
                                    range(mind, maxd),
                                    range(minh, maxh),
                                    range(minw, maxw),
                        ...
                        for idx in range(len(label_volumes)):
                            label_volumes[idx] = label_volumes[idx][
                                np.ix_(
                                    range(mind, maxd),
                                    range(minh, maxh),
                                    range(minw, maxw),
                ...
                if self.label_convert_source and self.label_convert_target:
                    label_volumes[0] = convert_label(
                        label_volumes[0],
                        self.label_convert_source,
                        self.label_convert_target,
                    )
                    # print(f'After converting, the unique number of the label is {np.unique(label_volumes[0])}')
            ...
            center_point = get_random_roi_sampling_center(
                volume_shape, sub_label_shape, batch_sample_model, boundingbox
            )
            sub_data = []
            for moda in range(len(transposed_volumes)):
                sub_data_moda = extract_roi_from_volume(
                    transposed_volumes[moda], center_point, sub_data_shape
                )
                if flip:
                    sub_data_moda = np.flip(sub_data_moda, -1)
                if down_sample_rate != 1.0:
                    sub_data_moda = ndimage.interpolation.zoom(
                        sub_data_moda, 1.0 / down_sample_rate, order=1
                    )
            ...
            sub_weight = extract_roi_from_volume(
                transposed_weight[0], center_point, sub_label_shape, fill="zero"
            )

            if flip:
                sub_weight = np.flip(sub_weight, -1)
            if down_sample_rate != 1.0:
                sub_weight = ndimage.interpolation.zoom(
                    sub_weight, 1.0 / down_sample_rate, order=1
                )
            weight_batch.append([sub_weight])
            if self.with_ground_truth:
                tranposed_label = transpose_volumes(label_volumes, slice_direction)
                sub_label = extract_roi_from_volume(
                    tranposed_label[0], center_point, sub_label_shape, fill="zero"
                )
                if flip:
                    sub_label = np.flip(sub_label, -1)
                if down_sample_rate != 1.0:
                    sub_label = ndimage.interpolation.zoom(
                        sub_label, 1.0 / down_sample_rate, order=0
                    )
        ...
        batch = {}
        batch["images"] = np.transpose(
            data_batch, [0, 2, 3, 4, 1]
        )  # axil 1 in the last, represent chans of t1 t1ce t2 flair
