...
def infer_ndims_from_file(file_path, loader=None):
    ...
    raise IOError(
        "could not get ndims from file header, please "
...
def correct_image_if_necessary(img):
    ...
    if not np.array_equal(
        np.sqrt(np.sum(np.square(img.affine[0:3, 0:3]), 0)), np.asarray(pixdim)
    ):
...
def do_resampling(data_array, pixdim_init, pixdim_fin, interp_order):
    ...
    try:
        ...
        tf.logging.fatal(
            "unknown pixdim format original %s output %s", pixdim_init, pixdim_fin
        )
        raise
    ...
    if len(data_shape) != 5:
        raise ValueError(
            "only supports 5D array resampling, " "input shape {}".format(data_shape)
        )
    data_resampled = []
    for time_point in range(0, data_shape[3]):
        ...
        for mod in range(0, data_shape[4]):
            data_new = scipy.ndimage.zoom(
                data_array[..., time_point, mod], to_multiply[0:3], order=interp_order
            )
...
def save_data_array(
    filefolder, filename, array_to_save, image_object=None, interp_order=3, reshape=True
):
    """
    ...
    if image_axcodes:
        array_to_save = do_reorientation(array_to_save, image_axcodes, dst_axcodes)
...
def save_volume_5d(img_data, filename, save_path, affine=np.eye(4)):
    ...
    try:
        if os.path.isfile(output_name):
            tf.logging.warning("File %s exists, overwriting the file.", output_name)
    ...
    try:
        ...
        for path in paths:
            ...
            if os.path.isfile(path_file_name):
                tf.logging.info("Resolving {} as {}".format(file_name, path_file_name))
...
def get_latest_subfolder(parent_folder, create_new=False):
    ...
    try:
        ...
        raise OSError
    log_sub_dirs = [name for name in log_sub_dirs if re.findall("^[0-9]+$", name)]
...
def image3(
    ...
    axis_order_all = axis_order + [
        i for i in range(len(tensor.shape.as_list())) if i not in axis_order
    ]
...
def infer_latest_model_file(model_dir):
    ...
    try:
        ...
        assert checkpoint, "checkpoint path not found " "in {}/checkpoint".format(
            model_dir
        )
        initial_iter = int(checkpoint.rsplit("-")[-1])
        tf.logging.info("set initial_iter to %d based " "on checkpoints", initial_iter)
        return initial_iter
